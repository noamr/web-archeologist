...
description: Use this skill for an efficient way to split/concat the HTML spec source (`whatwg/html`'s `source` file) when examining or editing it. Splitting the file makes reading, searching, and editing much faster and more reliable.
---
...
- **Reliable Editing**: Making precise edits using agent tools (`replace_file_content`, `Edit`) is far less error-prone on smaller files than on a multi-megabyte monolithic source file.
...
**Note on Paths**: In the commands below, `<path/to/html/source>` refers to the path to the HTML source file in the user's workspace, and `<skill-dir>` refers to the directory where this skill is installed (e.g., `~/.gemini/config/skills/html-spec-splitter` or `~/.claude/skills/html-spec-splitter`).
...
Locate the relevant section file in the `split_source` directory (e.g., `10_webappapis.html`). Use standard agent tools (`view_file`/`Read`, `replace_file_content`/`Edit`, `grep_search`/`Grep`) on these smaller files. This makes inference and context management much more efficient.
...
### 5. Clean Up
`split_source/` is scratch state, and `whatwg/html` does not gitignore it. Concatenate before building, diffing, or committing, then delete the directory and check `git status` to confirm the only remaining change is the intended edit to `source`:
```bash
rm -r <path/to/html>/split_source
```
